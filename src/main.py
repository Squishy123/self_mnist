...
for i in range(100):
    ...
    for j in range(20):
        ...
        j_class = torch.argmax(j_class, dim=1)

        by_labels[j_class].append(j_sample.detach().cpu().numpy())

        # TODO Fix this cause it wont work
        if j_class == j%10:
            score += 1

    ax2.scatter(i, score, color="blue")
    fig2.savefig("results/classifier_validation.png")
...
plt.close()
'''
model.load_state_dict(torch.load("results/classifier_training_9.pth"))
model.eval()
...
show_images(images, f"training_{9}_images.png", cols=20)

# 4. Generate Prototype Plot
x0 = by_labels[1][0].unsqueeze(0).to("cuda")
x1 = by_labels[1][1].unsqueeze(0).to("cuda")

_, x0_class = model(x0)
print(x0_class)
x0_class = torch.argmax(x0_class, dim=1)
_, x1_class = model(x1)
print(x1_class)
x1_class = torch.argmax(x1_class, dim=1)
